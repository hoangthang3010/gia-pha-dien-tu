...
export default function BookPage() {
  ...
  for (const ch of bookData.chapters) {
    const chapterPages = packChapterPages(
      ch.members,
      ch.generation,
      ch.romanNumeral,
    );
  ...
  if (totalNames <= APPENDIX_PAGE_LIMIT) {
    sections.push({
      ...
      pageNum: pageCounter++,
      appendixStart: 0,
      appendixEnd: totalNames,
      appendixIsFirst: true,
    });
  } else {
    let aStart = 0;
    let aPage = 0;
    while (aStart < totalNames) {
      const limit =
        aPage === 0 ? APPENDIX_PAGE_LIMIT - 20 : APPENDIX_PAGE_LIMIT;
  ...
  return (
    <div className="min-w-0 w-full overflow-hidden">
        ...
        <div className="px-4 py-2.5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground hidden sm:inline">
              {bookData.totalMembers} thành viên · {bookData.totalGenerations}{" "}
              đời · {sections.length} trang
            </span>
          </div>
            ...
            {sections.map((s) => (
              <div key={s.id} id={`preview-${s.id}`} className="relative">
                <div className="preview-card-inner bg-white rounded-lg shadow-md border overflow-hidden hover:shadow-xl transition-shadow">
                  <div
                    className="absolute top-2 right-2 z-10 bg-white/90 border rounded-full px-2 py-0.5 text-[10px] font-mono"
                  ...
                  <div className="aspect-[210/297] overflow-hidden">
                    ...
                    >
                      <BookSection
                        ...
                        bookData={bookData}
                        theme={t}
                        memberStart={s.memberStart}
                        memberEnd={s.memberEnd}
                        isFirstPage={s.isFirstPage}
                        appendixStart={s.appendixStart}
                        appendixEnd={s.appendixEnd}
                        appendixIsFirst={s.appendixIsFirst}
                      />
                    </div>
                  </div>
                  <div
                    className="px-3 py-2 border-t text-xs font-medium"
                    style={{ color: t.primary }}
                  >
                    {s.label}
                  </div>
      ...
      {!previewMode && (
        <div
          className="book-content max-w-[210mm] mx-auto bg-white"
        ...
        >
          ...
          {bookData.chapters.map((ch, ci) => (
            <section
              ...
              className="page-break px-8 py-12"
            >
          ...
          <section id="appendix" className="page-break px-8 py-12">
            <span className="page-label">
              Trang {bookData.chapters.length + 3}
            </span>
          ...
          <section
            id="closing"
            className="page-break px-8 py-16 text-center"
            style={{ fontFamily: "'Noto Serif', Georgia, serif" }}
          >
            <span className="page-label">
              Trang {bookData.chapters.length + 4}
            </span>
...
function BookSection({
  ...
  return (
    <div
      className="book-content px-12 py-12"
      style={{ fontFamily: "'Noto Serif', Georgia, serif", color: t.text }}
    >
      ...
      {(sectionId === "appendix" || sectionId.startsWith("appendix-")) && (
        <AppendixContent
          bookData={bookData}
          theme={t}
          startIdx={appendixStart ?? 0}
          endIdx={appendixEnd ?? bookData.nameIndex.length}
  ...
  return (
    <section className="cover-page flex flex-col items-center justify-center text-center px-8 py-16 min-h-[280mm]">
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        <h1
          className="text-4xl font-bold tracking-wider font-serif mb-2"
          style={{ color: t.primary }}
        >
          GIA PHẢ
        </h1>
        <h2
          className="text-5xl font-bold tracking-widest font-serif mb-8"
        ...
        <div
          className="space-y-2 text-lg font-serif"
          style={{ color: t.textMuted }}
        >
          <p>
            {bookData.totalGenerations} đời · {bookData.totalMembers} thành viên
          </p>
          <p className="text-base">
            {bookData.totalPatrilineal} người chính tộc
          </p>
        </div>
      </div>
...
function TocContent({
  ...
  return (
    <>
      <h2
        className="text-2xl font-bold text-center font-serif mb-8 tracking-wide"
        style={{ color: t.primary }}
      >
        MỤC LỤC
      </h2>
...
function ChapterContent({
  ...
  return (
    <>
      ...
      {showHeader === false && (
        <p
          className="text-xs mb-6 font-serif italic"
          style={{ color: t.textMuted }}
        >
...
function PersonEntry({
  ...
  return (
    <div
      className="person-entry rounded-lg p-4"
      style={{ border: `1px solid ${t.borderLight}`, background: t.primaryBg }}
    >
      ...
      <div className="ml-8 space-y-1 text-xs">
        {person.fatherName && (
          <div className="flex gap-1.5">
            <span className="w-12 flex-shrink-0" style={{ color: t.textMuted }}>
              Cha:
            </span>
            ...
            <ol className="mt-0.5 ml-3 space-y-0">
              {person.children.map((child, i) => (
                <li key={i} className="list-decimal" style={{ color: t.text }}>
                  <span
                    className={child.note ? "text-stone-500" : "font-medium"}
                  >
                    {child.name}
                  </span>
  ...
  return (
    <>
      {showHeader !== false && (
        <>
          <h2
            className="text-2xl font-bold text-center font-serif mb-2 tracking-wide"
            style={{ color: t.primary }}
          >
            PHỤ LỤC
          </h2>
          <p
            className="text-center font-serif mb-6"
            style={{ color: t.textMuted }}
          >
            Chỉ mục tên theo thứ tự A-Z
          </p>
          <div
      ...
      {showPatriHeader && start === 0 && (
        <h3
          className="text-base font-bold font-serif mb-3 tracking-wide pb-2"
          ...
          return (
            <div key={`e-${globalIdx}`}>
              ...
              <div className="flex items-baseline gap-1 py-0.5 break-inside-avoid">
                <span
                  className={
                    globalIdx < patriEnd ? "font-semibold" : "text-stone-600"
                  }
                  style={
                    globalIdx < patriEnd ? { color: t.primary } : undefined
                  }
                >
                  {entry.name}
                </span>
                <span
                  ...
                  style={{
                    borderColor:
                      globalIdx < patriEnd ? t.borderLight : "#d6d3d1",
                  }}
                />
                <span
                  className="text-xs"
                  style={{
                    color: globalIdx < patriEnd ? t.textMuted : "#a8a29e",
                  }}
                >
...
function ClosingContent({
  ...
  return (
    <>
      ...
      <div
        className="w-16 h-0.5 mx-auto mt-8 mb-6"
        style={{ background: t.primary }}
      />
      <p className="text-sm" style={{ color: t.textMuted }}>
        Gia phả dòng họ {bookData.familyName}
        <br />
        {bookData.exportDate}
      </p>
    </>
